...
public class ConversionServiceImpl implements ConversionService {
    ...
    public OutputDTO expressIntent(InputDTO inputDTO) {
        String output = findSavedResult(inputDTO.getInput());
...
//                """
        if (output == null) {
            ...
            output = languageModelService.generateContent(text);
        }
    ...
    private String findSavedResult(String input) {
        Long userId = securityUtils.getUserId();
        if (userId == null) return null;
        double threshold = 0.5;
        List<SavedResult> matchedResults = savedResultRepository.findByUserId(userId).stream().filter(s -> StringUtils.similarityScore(input, s.getInput()) > threshold).toList();
        if (matchedResults.isEmpty()) return null;
        return matchedResults.get(new Random().nextInt(matchedResults.size())).getOutput();
    }

    private String getTrainingSentences() {
        Map<String, String> map1 = Map.of("すみません、ちょっと…", "すみません、ちょっと都合が悪いです。", "田中さんは？", "田中さんはどこにいますか", "この資料、明日までに見ていただければと思います。", "この資料、明日までに見ていただければ助かると思います。", "私でよければ", "私でよければ手伝いますよ。", "それはちょっと", "それはちょっと難しいかもしれませんね。", "これについては", "これについては少し考えさせてください。", "「この映画、どうだった？」「うーん、でも、なんか…」", "「この映画、どうだった？」「うーん、でも、なんか最後がよく分からなかった」", "とりあえず、コーヒーでも飲みに行こうか…", "とりあえず、コーヒーでも飲みに行こうか…そのあと考えよう。", "「最近のプロジェクトはどう？」「まぁ、そんな感じで…」", "「最近のプロジェクトはどう？」「まぁ、そんな感じで…順調だけど、いろいろ大変。」", "「この資料、完成しましたか？」「一応…」", "「この資料、完成しましたか？」「一応形にはなっています」");
        Map<String, String> map2 = Map.of("もうちょっとだけ…", "もうちょっとだけ時間をください。", "まぁ、それはそれで…", "まぁ、それはそれでいいかもしれない。", "どういうか…", "どういうか…ちょっと複雑で説明が難しいですね。", "なんとかなるかな…", "なんとかなるかな…少し不安だけど。", "「この案について、どう思いますか？」「いちおう考えてみます…」", "「この案について、どう思いますか？」「いちおう考えてみます…ただ、他にも調べてみたいです。」");
...
//                .append("を出力して、\n"));
        sentences.forEach((key, value) -> sb.append("If the sentence is ").append("「").append(key).append("」").append(" rephrase it as ").append("「").append(value).append("」").append("\n"));
    ...
    public OutputDTO toEasyJapanese(InputDTO inputDTO) {
        ...
        String uuid = Optional.ofNullable(securityUtils.getUserId()).map(userId -> {
            History history = History.builder().userId(userId).action(ActionType.EASY_JAPANESE_MODE).input(inputDTO.getInput()).output(output).build();
            historyRepository.save(history);
            return history.getUuid();
        }).orElse(null);
    ...
    public OutputDTO translate(TranslateInputDTO inputDTO) {
        ...
        String uuid = Optional.ofNullable(securityUtils.getUserId()).map(userId -> {
            History history = History.builder().userId(userId).action(ActionType.TRANSLATION).sourceLanguage(inputDTO.getSourceLanguage()).targetLanguage(inputDTO.getTargetLanguage()).input(inputDTO.getInput()).output(output).build();
            historyRepository.save(history);
            return history.getUuid();
        }).orElse(null);
